...
### Flutter Proof Captures

| Capture                                         | Purpose                                                            |
| ----------------------------------------------- | ------------------------------------------------------------------ |
| ![Flutter login proof](ui-proof-login.png)      | Phone/email authentication surface with validation-ready inputs    |
| ![Flutter home proof](current-proof.png)        | Home/service hub proof captured from the Flutter UI                |
| ![Flutter emulator proof](emulator-proof-1.png) | Emulator capture used to verify mobile rendering in a device frame |

### Capture Checklist

- Seed realistic data before capturing list/detail surfaces; avoid empty states unless the empty state itself is being reviewed.
- Wait for Flutter hydration/network loading to settle before saving proof images.
- Keep new files under `docs/screenshots/` and reference them from this catalog plus the root README when they represent release-facing surfaces.
