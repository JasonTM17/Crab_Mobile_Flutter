...
- **Status casing fragility on socket events**. Socket payloads are matched against typed enums via `OrderStatus`/`RideStatus` parsers. Backend changes to status casing or string values (e.g. `OUT_FOR_DELIVERY` vs `out_for_delivery`) silently fall back to default branches in `FoodBloc` (`socket.on('order:status', ...)`) and `RideBloc`, leaving stale UI state. Treat status strings as a contract and bump the API contract whenever the backend changes them.
